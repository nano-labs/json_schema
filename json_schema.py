    ...
    def comparar(cls, item, item_schema):
        ...
        if isinstance(item_schema, dict):
            for chave, valor_schema in item_schema.items():
                valor = item.get(chave, "json_schema.None")
                if valor == "json_schema.None":
                    pprint(item)
                    raise Exception(u"Chave '%s' nao encontrada." % chave)
                    return False
        ...
        # Se for uma string ou unicode
            ...
            if not all_clear:
                raise Exception(u"Valores nao batem: '%s', '%s'" % (item, item_schema))
            return all_clear


def _teste(j):
    return loads(dumps(j)) == j
